...
class TrajectorySampler(nn.Module):
    ...
    def get_occupancy(self, generator, local_latents, trajectories):
        ...
        occupancy = occupancy.view(B, -1)
        return occupancy

    def get_validation(self, generator, local_latents, trajectories):
        B = local_latents.shape[0]
        query_points = trajectories.unsqueeze(0).expand(B, -1, -1)#, -1)
        query_points = query_points.to(local_latents.device)
        ...
        if generator.coordinate_scale is not None:
            query_points = query_points/(generator.coordinate_scale / 2)
        # get occupancies for all trajectories
        with torch.no_grad():
            # z is tensor for shape [B, z_dim]
            sampled_local_latents, local_latents = generator.sample_local_latents(local_latents=local_latents, xyz=query_points.unsqueeze(1))
        #sampled_local_latents, local_latents = self.sample_local_latents(local_latents, xyz=xyz)

        #occupancy = occupancy.view(B, -1, self.seq_len)  # [B, n_trajectories, seq_len]
        #occupancy = torch.sum(occupancy, dim=2)  # [B, n_trajectories]
        occupancy = sampled_local_latents.mean(dim=1).view(B, -1)
        return occupancy
    ...
    def sample_trajectories(self, generator, local_latents):
        ...
        trajectories = torch.randn((real_Rts.shape[0],real_Rts.shape[1],3)).to(real_Rts.device) * 2
        trajectories[:,:,1] = 0
        ...
        if self.mode == 'sample':
            occupancy = self.get_occupancy(generator=generator, local_latents=local_latents, trajectories=trajectories.view(-1,3))
            validancy = self.get_validation(generator=generator, local_latents=local_latents, trajectories=trajectories.view(-1,3))

            Rts = []
            for b in range(B):
                valid_indices = torch.where(validancy[b].abs() > 0.01)[0].tolist()
                if len(valid_indices) < 10:
                    valid_indices = torch.where(validancy[b].abs() > 0.000)[0].tolist()
                    if len(valid_indices) < 10:
                        valid_indices = torch.where(validancy[b].abs() >= 0.000)[0].tolist()
            # randomly choose 1k trajectories to sample from
                n_subsamples = min(len(valid_indices), 1000)
                subset_indices = np.random.choice(valid_indices, n_subsamples,replace=False)
                sample_weights = nn.functional.softmin(occupancy[b, subset_indices] + 1e-8, dim=-1)
                nans = torch.isnan(sample_weights)
                sample_weights[nans] = 1e-7
                selected_indices = torch.multinomial(sample_weights, num_samples=1, replacement=False)#.squeeze(1)
                Rts_ = real_Rts.view(-1, 4,4)[subset_indices][selected_indices].unsqueeze(1)
                Rts_ = Rts_.inverse()
                Rts_[:,:, :3, 3] = trajectories.view(-1,3)[subset_indices][selected_indices]
                Rts_ = Rts_.inverse()
                Rts.append(Rts_)
            Rts = torch.cat(Rts)
